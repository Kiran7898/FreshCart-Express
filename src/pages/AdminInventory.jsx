...
import { Shield, TrendingUp, Users, ShoppingBag, Plus, Percent, AlertTriangle, Calendar, ClipboardList, Trash2, ArrowRight, RefreshCw } from "lucide-react";
...
export const AdminInventory = () => {
  ...
  // Add batch inputs
  const [batchCode, setBatchCode] = useState("BAT-" + Math.random().toString(36).substr(2, 6).toUpperCase());
  const [batchQty, setBatchQty] = useState("35");
  ...
  return (
    <div className="space-y-6 animate-fade">
      ...
      {/* Nav Tabs Bar */}
      <div className="flex border-b border-gray-200 gap-1 flex-wrap font-sans">
        <button
          ...
          {loadingMetrics ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div className="bg-white rounded-xl border border-gray-150 p-5 flex items-center justify-between shadow-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                  ...
                  <div className="space-y-4">
                    {metrics.categorySales && metrics.categorySales.length > 0 ? (
                      metrics.categorySales.map((c) => {
                        ...
                        return (
                          <div key={c.name} className="space-y-1.5 font-sans">
                            <div className="flex justify-between text-xs font-bold text-slate-700">
                              <span>{c.name}</span>
                              <span className="text-slate-500 font-mono">₹{c.sales.toFixed(2)}</span>
                            </div>
                      ...
                      <span className="text-xs text-slate-400 italic font-sans">No category sales recorded</span>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-150 rounded-xl p-5 md:col-span-5 text-left shadow-xs">
                  <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-400 mr-2 flex items-center gap-1.5 mb-4 font-sans">
                  ...
                  <div className="divide-y divide-gray-100 max-h-[280px] overflow-y-auto">
                    {metrics.topSellingProducts && metrics.topSellingProducts.length > 0 ? (
                      metrics.topSellingProducts.map((p, idx) => (
                        <div key={idx} className="flex justify-between items-center py-2.5 font-sans">
                ...
                <div className="bg-white border border-gray-150 rounded-xl p-5 md:col-span-3 text-left shadow-xs">
                  ...
                  <div className="space-y-3 font-semibold text-xs text-slate-500 font-sans">
      ...
      {activeTab === "audit" && (
        <div className="space-y-4 text-left">
          <div className="flex justify-between items-center flex-wrap gap-2">
            <div>
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider font-sans">Edible Batch Expiration Ledger</h3>
          ...
          {loadingAudit ? (
            ...
            <div className="border border-gray-150 rounded-xl overflow-hidden bg-white shadow-xs">
              <table className="min-w-full divide-y divide-gray-150 animate-fade" id="audit-table">
                <thead className="bg-[#f9fafb]">
                  <tr className="text-left text-[10px] font-black text-slate-400 uppercase tracking-wider font-mono">
                    <th scope="col" className="px-4 py-3 border-0 bg-transparent">Gross Item Name</th>
                    <th scope="col" className="px-4 py-3 border-0 bg-transparent">Deptartment</th>
                    <th scope="col" className="px-4 py-3 text-center border-0 bg-transparent">Batch ID</th>
                    <th scope="col" className="px-4 py-3 text-center border-0 bg-transparent">Remaining Quantity</th>
                    <th scope="col" className="px-4 py-3 text-center border-0 bg-transparent">Edible Expiry Date</th>
                    <th scope="col" className="px-4 py-3 text-center border-0 bg-transparent">Days left</th>
                    <th scope="col" className="px-4 py-3 text-center border-0 bg-transparent">Calculated ERF</th>
                    <th scope="col" className="px-4 py-3 text-right border-0 bg-transparent">Clearance Actions</th>
                  </tr>
                </thead>
                  ...
                  {auditItems.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50">
                        <td className="px-4 py-3.5 text-left font-sans font-bold text-slate-800 border-0">{item.productName}</td>
                        <td className="px-4 py-3.5 text-left font-sans border-0">{item.category}</td>
                        <td className="px-4 py-3.5 text-center font-bold text-slate-500 border-0">#{item.batchNumber}</td>
                        <td className="px-4 py-3.5 text-center font-extrabold text-slate-700 border-0">{item.quantity} units</td>
                        <td className="px-4 py-3.5 text-center border-0">{item.expiryDate}</td>
                        <td className="px-4 py-3.5 text-center font-bold text-slate-700 border-0">{item.daysRemaining} days</td>
                        <td className="px-4 py-3.5 text-center border-0">
                          <span className={`px-2 py-0.5 rounded-md border text-[10px] ${risk.bg}`}>
                            {item.expiryRiskFactor} ({risk.text})
                          </span>
                        </td>
                        <td className="px-4 py-3.5 text-right font-sans border-0">
                          <button
                            onClick={() =>
                              setDiscountModal({
                                open: true,
      ...
      {activeTab === "add_product" && (
        <form onSubmit={handleCreateProductSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-8 text-left bg-white border border-gray-150 rounded-2xl p-6 shadow-sm animate-fade">
          ...
          <div className="md:col-span-7 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold focus:outline-none focus:border-green-500 focus:bg-white transition-all cursor-pointer border-0"
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                className="bg-slate-55 border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold focus:outline-none focus:border-green-500 focus:bg-white transition-all resize-none"
          ...
          <div className="md:col-span-5 bg-slate-55 border border-slate-150 rounded-xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5 leading-none">
            ...
            <div className="space-y-3 p-3 bg-white border border-slate-200/50 rounded-lg">
              <div className="grid grid-cols-2 gap-2.5">
                <div className="flex flex-col gap-1 text-[11px] font-semibold text-slate-500">
                  <span className="text-[9px] font-black text-slate-400 uppercase font-mono">Batch Code</span>
                  <input
                    type="text"
                    value={batchCode}
                ...
                <div className="flex flex-col gap-1 text-[11px] font-semibold text-slate-500 font-sans">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="flex flex-col gap-1 text-[11px] font-semibold text-slate-500 font-sans">
                  <span className="text-[9px] font-black text-slate-400 uppercase font-mono">Mfg Date</span>
                  <input
                    type="date"
                    value={batchMfgDate}
                ...
                <div className="flex flex-col gap-1 text-[11px] font-semibold text-slate-500 font-sans">
                  <span className="text-[9px] font-black text-slate-400 uppercase font-mono">Expiration Date</span>
                  <input
                    type="date"
                    value={batchExpDate}
            ...
            <div className="space-y-2">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block leading-none font-sans">Staged Ingest Batches ({batchesList.length})</span>
              
              <div className="space-y-1.5 max-h-[150px] overflow-y-auto" id="payload-batches-shelf-list">
                {batchesList.length === 0 ? (
                  <span className="text-xs text-slate-400 italic block py-4 text-center font-sans">Batch shelf listing empty. Build at least one batch above.</span>
                ) : (
                  batchesList.map((b, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-white border border-slate-150 rounded px-3 py-1.5 text-xs font-mono font-semibold text-slate-600 hover:shadow-xs">
                      ...
                      <button
                        ...
                        className="p-1 hover:bg-red-50 text-red-500 hover:text-red-700 rounded cursor-pointer border-0 bg-transparent animate-fade"
                        title="Remove batch"
                      >
      ...
      {discountModal && discountModal.open && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center p-4 z-50 animate-fade">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 text-left shadow-xl space-y-4 relative border border-slate-100">
            <div>
              <span className="text-[10px] font-black text-red-600 uppercase tracking-wider flex items-center gap-1 font-sans">
                <Percent className="w-3.5 h-3.5" />
                <span>Auto Clearance Action</span>
              </span>
              <h3 className="text-base font-bold text-slate-800 mt-1 font-sans">Clearance Price Reduction: {discountModal.name}</h3>
            ...
            <div className="flex flex-col gap-1.5 bg-slate-50 rounded-xl p-4 border border-slate-155 font-sans">
